...
public class UpdateFaculty extends JFrame implements ActionListener {
    ...
    public String getEducation(){ return textcourse.getText(); }
    public String getDept()     { return textbranch.getText(); }
    public String getEmpId()    { return empText.getText(); }
    ...
    public void setEmpIdText(String v)    { empText.setText(v); }
    public void setEducationText(String v){ textcourse.setText(v); }
    public void setDeptText(String v)     { textbranch.setText(v); }
    ...
    UpdateFaculty() {
        setSize(1200, 900); setLocation(250, 0); setLayout(null);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); getContentPane().setBackground(Color.WHITE);
        ImageIcon ic = new ImageIcon(ClassLoader.getSystemResource("white.png")); setIconImage(ic.getImage());

        ImageIcon icon1 = new ImageIcon(ClassLoader.getSystemResource("updateteacher.png"));
        ...
        ImageIcon icon = new ImageIcon(ClassLoader.getSystemResource("updateFaculty.png"));
        ...
        formPanel.add(createLabel("Search Employee ID:", 260, 30));
        cEMPID = new Choice(); cEMPID.setBounds(450, 35, 120, 30); formPanel.add(cEMPID);
        ...
        formPanel.add(createLabel("Employee ID", 70, 130));   empText     = createDisplayLabel(200, 130, formPanel);
        ...
        formPanel.add(createLabel("Qualification", 70, 330)); textcourse  = createField(200, 330, formPanel);
        formPanel.add(createLabel("Department", 470, 330));   textbranch  = createField(610, 330, formPanel);
    ...
    private void fetchData() {
        try {
            ...
            DatabaseConnection c = new DatabaseConnection();
            PreparedStatement ps = c.prepare("SELECT * FROM teacher WHERE empId = ?");
            ...
            while (rs.next()) {
                ...
                setEmpIdText(rs.getString("empId"));
                setEducationText(rs.getString("education"));
                setDeptText(rs.getString("department"));
            }
            rs.close(); ps.close();
        } catch (Exception e) { e.printStackTrace(); }
    }
        ...
        if (e.getSource() == submit) {
            try {
                ...
                PreparedStatement ps = c.prepare(
                        "UPDATE teacher SET address=?, phone=?, email=?, education=?, department=?, aadhar=? WHERE empId=?");
                ps.setString(1, getAddress());
                ps.setString(2, getPhone());
                ps.setString(3, getEmail());
                ps.setString(4, getEducation());
                ps.setString(5, getDept());
                ps.setString(6, getAadhar());
                ps.setString(7, getEmpId());
                ps.executeUpdate();
                ps.close();
                JOptionPane.showMessageDialog(null, "Teacher Profile Updated Successfully");
                setVisible(false);
            } catch (Exception ex) { ex.printStackTrace(); }
        } else { setVisible(false); }
    }

    public static void main(String[] args) { new UpdateFaculty(); }
}
